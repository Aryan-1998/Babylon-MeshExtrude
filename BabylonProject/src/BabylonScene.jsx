...
const Hover_Color = new BABYLON.Color4(1, 1, 0, 1);
const Ghost_Color = new BABYLON.Color4(0.5, 0.5, 0, 1);

function findSharedFacesByVertexPositions(indices,positions){

    // Create an array to store the indices of faces that share the same vertex positions
    const sharedFaces=Array.from({length:indices.length},() => []);
    

    // Iterate through each index in the indices array
    for (let i = 0; i < indices.length; i++) {
        for (let j = 0; j < indices.length; j++) {
             // Check if the vertex positions at the corresponding indices are the same
          if (
            ...
            // Push the index of the face that shares the same vertex positions into the sharedFaces array
            sharedFaces[indices[i]].push(indices[j]);
          }
        }
    }
      // Return the array containing the indices of faces that share the same vertex positions
      return sharedFaces;
...
const BabylonScene = () => {
    ...
    useEffect(() => {
      ...
      const sharedFaces = findSharedFacesByVertexPositions(indices, positions);
  
      // Creating a camera object and set its initial position and target
      ...
      //creating a hemispheric light in the scene
      const light=new BABYLON.HemisphericLight("light", new BABYLON.Vector3(0, 10, 0), scene);
      light.intensity=0.8;
      // Adding an event listener to the window and resizing the engine/canvas accordingly
      ...
      // creating a GUI with a button that resets the mesh to original form
